    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            if created_user.type == "SELLER":
                ...
                created_user.user_permissions.add(*permissions)
            ...
            message = get_template("authentication/signup_email.html").render(
                context
            )

            def send_email(
                to_list, subject, message, sender="jainsagar619@gmail.com"
            ):
...
class HomeView(TemplateView):
    def get(self, request, *args, **kwargs):
        ...
        # if request.user.has_perms(['product.add_product', 'product.change_product', 'product.view_product', 'product.delete_product']):
        if request.user.groups.filter(name='seller').exists():
            ...
            clothes = Product.objects.filter(
                user=request.user, category__name__iexact="clothes"
            )[:5]
            footwares = Product.objects.filter(
                user=request.user, category__name__iexact="footwares"
            )[:5]
            home_appliances = Product.objects.filter(
                user=request.user, category__name__iexact="home appliances"
            )[:5]
        else:
            electrical_appliances = Product.objects.filter(
                category__name__iexact="Electrical Appliances"
            )[:5]
            clothes = Product.objects.filter(category__name__iexact="clothes")[
                :5
            ]
            footwares = Product.objects.filter(
